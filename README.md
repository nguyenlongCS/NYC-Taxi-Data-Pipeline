...
├── load_parquet_to_staging.py # Nạp Parquet (output Spark) -> staging (COPY)
└── requirements.txt
```
...
| [`docs/troubleshooting.md`](docs/troubleshooting.md) | Các lỗi thực tế đã gặp và cách xử lý (FK constraint, race condition, Docker volume, Spark OOM...) |
...
| **Airflow** | Đóng gói pipeline thành 1 DAG chạy theo lịch, tự retry khi lỗi | ⏳ Tiếp theo |
